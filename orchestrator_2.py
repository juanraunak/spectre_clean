...
def collect_user_inputs() -> Optional[Dict[str, Any]]:
    ...
    # Company name
    company = input("\n[1/5] Enter company name (or 'quit'): ").strip()
    if company.lower() in ("quit", "exit", "q"):
        return None
    
    # Number of employees for SHADE
    n = input("[2/5] Number of employees for SHADE [50]: ").strip()
    num_employees = int(n) if n.isdigit() else 50
    
    # MIRAGE settings
    run_mirage = input("[3/5] Run MIRAGE competitor analysis? (y/n): ").strip().lower() in ("y", "yes")
    competitors = 10
    ...
    # CIPHER settings
    run_cipher = input("[4/5] Run CIPHER (skills extraction)? (y/n): ").strip().lower() in ("y", "yes")
    ...
    print(f"[5/5] Pipeline will auto-run remaining agents after CIPHER")
    if auto_agents_info:
